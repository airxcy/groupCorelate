...
__global__ void renderGroup(unsigned char* d_renderMask,float* d_curvec ,int* d_label,unsigned char* d_clrvec,float* d_persMap,int* d_neighbor)
{
    int pidx=blockIdx.x;
...
__global__ void searchNeighborMap(int* d_neighbor,float* d_cosine,float* d_velo, ofv* d_ofvec,float* d_distmat ,float * d_persMap, int nFeatures)
{
    ...
    if (r < c)
    {
        ...
        float persval=0;
        int ymin=min(d_ofvec[r].y1,d_ofvec[c].y1),xmin=min(d_ofvec[r].x1,d_ofvec[c].x1);
        ...
        //if (dx < wrange && dy < hrange)
        {
            ...
            //if(norm0>1&&norm1>1)
            {
                ...
                dist=dist/(1+abs(dist))-0.5;
                //dist=-((dist > wrange) - (dist < wrange));
                d_distmat[yidx*nFeatures+xidx]=dist*distdecay+d_distmat[yidx*nFeatures+xidx]*(1-distdecay);
                d_distmat[xidx*nFeatures+yidx]=dist*distdecay+d_distmat[xidx*nFeatures+yidx]*(1-distdecay);
                d_cosine[yidx*nFeatures+xidx]=cosine*cosdecay+d_cosine[yidx*nFeatures+xidx]*(1-cosdecay);
                d_cosine[xidx*nFeatures+yidx]=cosine*cosdecay+d_cosine[xidx*nFeatures+yidx]*(1-cosdecay);
                d_velo[yidx*nFeatures+xidx]=veloCo*velodecay+d_velo[yidx*nFeatures+xidx]*(1-velodecay);
                d_velo[xidx*nFeatures+yidx]=veloCo*velodecay+d_velo[xidx*nFeatures+yidx]*(1-velodecay);
                if(d_distmat[yidx*nFeatures+xidx]>0.1&&d_cosine[yidx*nFeatures+xidx]>0.4&&d_velo[yidx*nFeatures+xidx]<0.5)
                {
                ...
                else
                {
                    ...
                    d_neighbor[xidx*nFeatures+yidx]=0;
                }
            }
        }
    }
...
int KLTsparse_CUDA::init(int w, int h,unsigned char* framedata,int nPoints)
{
    ...
    gpu_zalloc(d_neighbor,nFeatures*nFeatures,sizeof(int));
    h_neighbor =(int *)zalloc(nFeatures*nFeatures,sizeof(int));
    h_distmat = (float *)zalloc(nFeatures*nFeatures,sizeof(float));
    gpu_zalloc(d_distmat,nFeatures*nFeatures,sizeof(float));
    h_cosine = (float *)zalloc(nFeatures*nFeatures,sizeof(float));
    gpu_zalloc(d_cosine,nFeatures*nFeatures,sizeof(float));
    h_velo = (float *)zalloc(nFeatures*nFeatures,sizeof(float));
    gpu_zalloc(d_velo,nFeatures*nFeatures,sizeof(float));
    ...
    groupVelo = (float*)zalloc(nFeatures*2,sizeof(float));
    //Bounding Box
...
void KLTsparse_CUDA::filterTrack()
{
    ...
    for (int k = 0; k < nFeatures; k++)
    {
        ...
        if(lost)
        {
            ...
            memset(h_neighbor+k*nFeatures,0,nFeatures*sizeof(int));
            for(int i=0;i<nFeatures;i++)h_neighbor[i*nFeatures+k]=0;
            memset(h_distmat+k*nFeatures,0,nFeatures*sizeof(float));
            for(int i=0;i<nFeatures;i++)h_distmat[i*nFeatures+k]=0;
            memset(h_cosine+k*nFeatures,0,nFeatures*sizeof(float));
            for(int i=0;i<nFeatures;i++)h_cosine[i*nFeatures+k]=0;
            memset(h_velo+k*nFeatures,0,nFeatures*sizeof(float));
            for(int i=0;i<nFeatures;i++)h_velo[i*nFeatures+k]=0;
        }
...
void KLTsparse_CUDA::Render(unsigned char * framedata)
{
    ...
    */
    ...
    {
        int x=h_bbox[curTrkingIdx*4]+0.5,y=h_bbox[curTrkingIdx*4+1]+0.5,
        x1=h_bbox[curTrkingIdx*4+2]+h_bbox[curTrkingIdx*4]+0.5,y1=h_bbox[curTrkingIdx*4+3]+h_bbox[curTrkingIdx*4+1]+0.5;
        ...
        if(rect.area()>10)
        {
            ...
            resize(patchMat,tmpmat,Size(zoomW,zoomH));
            //imshow("patch",tmpmat);
        }
    }
    ...
    renderGroup<<<nFeatures,nFeatures>>>(d_renderMask,d_curvec ,d_label,d_clrvec,d_persMap,d_neighbor);
...
int KLTsparse_CUDA::updateAframe(unsigned char* framedata, int fidx)
{
    ...
    if(groupOnFlag&&ofvBuff.len>0)
    {
        ...
        cudaMemcpy(d_distmat,h_distmat,nFeatures*nFeatures*sizeof(float),cudaMemcpyHostToDevice);
        cudaMemcpy(d_cosine,h_cosine,nFeatures*nFeatures*sizeof(float),cudaMemcpyHostToDevice);
        cudaMemcpy(d_velo,h_velo,nFeatures*nFeatures*sizeof(float),cudaMemcpyHostToDevice);
        cudaMemset(d_ofvec, 0, nFeatures*sizeof(ofv));
        cudaMemcpy(d_ofvec, ofvBuff.data, ofvBuff.len*sizeof(ofv), cudaMemcpyHostToDevice);

        searchNeighborMap <<<ofvBuff.len, ofvBuff.len >>>(d_neighbor,d_cosine,d_velo,d_ofvec,d_distmat,d_persMap, nFeatures);
        cudaMemcpy(h_neighbor,d_neighbor,nFeatures*nFeatures*sizeof(int),cudaMemcpyDeviceToHost);
        cudaMemcpy(h_distmat,d_distmat,nFeatures*nFeatures*sizeof(float),cudaMemcpyDeviceToHost);
        cudaMemcpy(h_cosine,d_cosine,nFeatures*nFeatures*sizeof(float),cudaMemcpyDeviceToHost);
        cudaMemcpy(h_velo,d_velo,nFeatures*nFeatures*sizeof(float),cudaMemcpyDeviceToHost);
...
//                framedata[(prelabel*frame_width+label)*3]=h_overlap[prelabel*nFeatures+label]/float(nFeatures)*255;
//                framedata[(prelabel*frame_width+label)*3+1]=h_overlap[prelabel*nFeatures+label]/float(nFeatures)*255;
//                framedata[(prelabel*frame_width+label)*3+2]=h_overlap[prelabel*nFeatures+label]/float(nFeatures)*255;
            }
        }
        rgbMat.upload(curframe);
        reGroup();
        ...
        for(int i=0;i<nFeatures;i++)
        {
            ...
            if(gidx)
            {
                ...
                {
                    groupVelo[gidx*2]+=trackBuff[i].cur_frame_ptr->x-trackBuff[i].getPtr(trackBuff[i].len-2)->x;
                    groupVelo[gidx*2+1]+=trackBuff[i].cur_frame_ptr->y-trackBuff[i].getPtr(trackBuff[i].len-2)->y;
                }
        ...
        for(int i=1;i<=maxgroupN;i++)
        {
            ...
            if(h_gcount[i]>0)
            {
                ...
                //group velocity
                groupVelo[i*4+2]=groupVelo[i*4+2]/float(h_gcount[i]);
                groupVelo[i*4+3]=groupVelo[i*4+3]/float(h_gcount[i]);
                if(calPolyGon)
                {
        ...
        if(updateFlag)
        {
            ...
            for(int i=1;i<=maxgroupN;i++)
            {
                if(h_gcount[i]>0)
                {
                    ...
                    if(h_bbstats[i*4]*3/4<h_bbstats[i*4+1])h_bbstats[i*4]=h_bbstats[i*4+1]*4/3;
                    else h_bbstats[i*4+1]=h_bbstats[i*4]*3/4;
                }
            }
        }
...
void KLTsparse_CUDA::knn()
{
    for(int i=0;i<ofvBuff.len;i++)
    {
        ...
        {
            if(h_neighbor[ridx*nFeatures+j])
            {
        ...
        memset(h_neighbor+ridx*nFeatures,0,nFeatures);
        for(int k=0;k<KnnK;k++)
        {
            if(h_KnnIdx[k]>=0)
            {
                h_neighbor[ridx*nFeatures+h_KnnIdx[k]]=1;
            }
        }
    }
}
